...
trait StreamHandler
{
    ...
    private function log(int $event, float $time, string $path, string $call, array $params = [], $return = null): void
    {
        ...
        $timeFormatted = Ansi::color('(' . round($time * self::TIME_MULTIPLIER) . ' ' . self::TIME_UNIT . ')', Dumper::$colors['time']);
    ...
    public function url_stat(string $path, int $flags)
    {
        ...
        try {
            ...
            $this->log(self::STAT, $this->time, $path, 'url_stat', [$flags], $this->formatStat($result));
        }
